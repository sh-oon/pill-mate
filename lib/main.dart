...
Future<void> main() async {
  ...
  // 알림 인프라 init은 await — launch details + flush + syncAll 순서 보장.
  //  1) notification service init
  //  2) cold start deep link 추출 (init 직후여야 정확)
  //  3) 백그라운드 isolate에서 큐에 적재된 pending 액션 flush
  //  4) DB 기반 스케줄을 시스템 알림에 동기화 (앱 재설치/업데이트 대비)
